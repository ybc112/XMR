// 构建并合并 admin-panel 到 frontend/dist/admin（Vercel /admin 子路径部署）
...
const target = path.join(frontendDir, 'dist', 'admin');

// 1. 构建 admin-panel（base: /admin/，Router basename: /admin）
...
// 2. 复制到 frontend/dist/admin
console.log('[merge-admin] 合并到 dist/admin ...');
fs.rmSync(target, { recursive: true, force: true });
fs.cpSync(path.join(adminDir, 'dist'), target, { recursive: true });
console.log('[merge-admin] 完成 ✅ 后台将部署在 /admin 路径');
